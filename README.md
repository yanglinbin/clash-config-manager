...
-  **订阅链接模式（默认）** - 生成时把订阅链接写入 proxy-providers，节点由客户端拉取，服务器无需访问订阅源
...
默认生成时只把 `active_provider` 的订阅链接写入 `proxy-providers`
（顶部 `proxy-providers:` 字段），节点由 Clash/Mihomo 客户端按配置的间隔
自行拉取，服务器不需要访问订阅源；地区组通过策略组 `use` + `filter`
按节点名关键词过滤实现。

如需回到“服务器拉取并内嵌节点”的方式，可在 `[clash]` 中设置
`embed_nodes = true`（内嵌模式下地区组才会按实际节点显式列出）。

### 地区组

为 `[regions]` 中每个地区生成一个地区组（如 `香港`）；默认模式下
通过 `use` + `filter` 按节点名关键词匹配，`embed_nodes = true` 时
按服务器实际拉取到的节点显式列出。
